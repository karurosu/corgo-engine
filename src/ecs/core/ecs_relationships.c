...
CE_Result CE_Entity_AddRelationship_Internal(INOUT CE_ECS_Context* context, IN CE_Id entity, IN CE_Id relationshipToAdd, OUT_OPT CE_ERROR_CODE* errorCode)
{
    ...
    result = CE_Bitset_setBit(&entityData->m_entityRelationshipBitset, CE_Id_getRelationshipTypeId(relationshipToAdd));
    if (result != CE_OK) {
...
CE_Result CE_Entity_AddRelationship(INOUT CE_ECS_Context* context, IN CE_Id entity, IN CE_TypeId relationshipType, IN CE_Id targetEntity, OUT_OPT CE_ERROR_CODE* errorCode)
{
    ...
    result = CE_Entity_AddRelationship_Internal(context, entity, newRel, errorCode);
    if (result != CE_OK) {
        return CE_ERROR;
    }

    // Make reciprocal relationship
    result = CE_Id_make(CE_ID_ENTITY_RELATIONSHIP_KIND, CE_RELATIONSHIPS_RECIPROCALS[relationshipType], CE_Id_getGeneration(entity), CE_Id_getUniqueId(entity), &newRel);
    if (result != CE_OK) {
        CE_SET_ERROR_CODE(errorCode, CE_ERROR_CODE_INTERNAL_ERROR);
        return CE_ERROR;
    }

    result = CE_Entity_AddRelationship_Internal(context, targetEntity, newRel, errorCode);
    if (result != CE_OK) {
        return CE_ERROR;
    }
...
CE_Result CE_Entity_RemoveRelationship(INOUT CE_ECS_Context* context, IN CE_Id entity, IN CE_TypeId relationshipType, IN CE_Id targetEntity, OUT_OPT CE_ERROR_CODE* errorCode)
{
    ...
    result = CE_Entity_RemoveRelationship_Internal(context, entity, newRel, errorCode);
    if (result != CE_OK) {
        return CE_ERROR;
    }
    ...
    result = CE_Entity_RemoveRelationship_Internal(context, targetEntity, newRel, errorCode);
    if (result != CE_OK) {
        return CE_ERROR;
    }
