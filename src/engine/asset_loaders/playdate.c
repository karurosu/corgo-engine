...
// Bitmap asset loader
CE_DEFINE_ASSET_LOADER_LOAD(CE_ASSET_TYPE_BITMAP)
{
...
CE_DEFINE_ASSET_LOADER_FREE(CE_ASSET_TYPE_BITMAP)
{
    PlaydateAPI* pd = CE_GetPlaydateAPI();
    if (!pd || !asset)
    {
...
// Font asset loader
CE_DEFINE_ASSET_LOADER_LOAD(CE_ASSET_TYPE_FONT)
{
    PlaydateAPI* pd = CE_GetPlaydateAPI();
    if (!pd || !assetPath)
    {
...
CE_DEFINE_ASSET_LOADER_FREE(CE_ASSET_TYPE_FONT)
{
    PlaydateAPI* pd = CE_GetPlaydateAPI();
    if (!pd || !asset)
    {
